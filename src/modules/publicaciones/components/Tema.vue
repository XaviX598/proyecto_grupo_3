<template>
  <div class="conversacion">
    <div class="titulo-descripcion">
      ...
      <br>
      <label class="encabezado2" for="">Descripción: {{ temaobjeto.descripcion }}</label>
      <br>
    ...
    <div class="container">
      <div class="comentarios">
        <div class="comentario" v-for="(comentario, index) in comentarios" :key="index"
          :class="{ 'comentario-derecha': comentario.usuario.usuario !== usuarioActual }">
      ...
      <div class="agregar-comentario">
        <textarea placeholder="Ingrese su comentario" v-model="comentario" class="comentario-textarea"></textarea>
        <!-- <button @click="ingresarComentario" class="comentario-button">Ingresar Comentario</button> -->
        <font-awesome-icon icon="fa-solid fa-paper-plane" @click="ingresarComentario" class="comentario-button" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async actualizarComentarios() {
      this.buscarTodos(); // Actualizar la lista de comentarios
    }
  },
